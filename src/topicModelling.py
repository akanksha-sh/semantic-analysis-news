...
def topic_modelling(data_words, max_topics=10, min_topics=2):
    ...
    n_topics_coherence = [
        get_coherences(data_words, corpus_tfidf, lda_dictionary, n)
        for n in range(min_topics, max_topics)
    ]
...
def check_invalid(
    kw_doc, sp, stopwords, model=None, model_check=False, allowed_pos=["NOUN"]
):
    kw = list(sp(kw_doc))[0]
...
def get_topic_name(keywords, sp, embedding_model, stopwords):
    ...
    fw = [
        w
        for w in words
        if not check_invalid(w, sp, stopwords, model=embedding_model, model_check=True)
    ][:5]
    topic_names = embedding_model.most_similar_cosmul(positive=fw, topn=5)
    filtered_topic_names = [
        tn[0] for tn in topic_names if not check_invalid(tn[0], sp, stopwords)
    ]
...
def get_topic_data(
    cid, topic_doc_mapping, doc_sentiments, lda_model, sp, embedding_model, stopwords
):
    topics_data = []
