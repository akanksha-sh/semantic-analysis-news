...
def dataloader():
    ...
    filtered_data_groups = [
        ...
        if len(data_groups.get_group(x)) > mean_count
    ]
...
def run(cf, sp, word_embedding_model, pickled_ner, sentiment_predictor, input_group):
    ...
    vectorized_intros = np.array(
        clustering.vectorize(filtered_tokens, model=word_embedding_model)
    )
    ...
    print("Time to run one group", file_suffix, time.time() - start)
