...
def dataloader():
    ...
    filtered_data_groups = [
        data_groups.get_group(x)
        for x in data_groups.groups
        if len(data_groups.get_group(x)) > threshold
    ]
...
def load_models():
    ...
    sentiment_predictor = Predictor.from_path(
        "/vol/bitbucket/as16418/tempFolder/models/roberta-sentiment.tar.gz",
        cuda_device=torch.cuda.current_device(),
    )
    print("sentiment model loaded")

    cf = Predictor.from_path(
        "/vol/bitbucket/as16418/tempFolder/models/coref-spanbert-large.tar.gz",
        cuda_device=torch.cuda.current_device(),
    )
    print("coref model loaded")

    ner_predictor = Predictor.from_path(
        "/vol/bitbucket/as16418/tempFolder/models/fine-grained-ner.tar.gz",
        cuda_device=torch.cuda.current_device(),
    )
